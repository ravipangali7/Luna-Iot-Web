...
const GeofenceCreatePage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showSuccess('Geofence created successfully!');
      navigate(`/alert-system/${instituteId}`);
    } catch (err: unknown) {
      console.error('Error creating geofence:', err);
      const errorMessage = (err as any)?.response?.data?.message || 'Failed to create geofence. Please try again.';
    ...
    <Container>
      <div className="mb-6">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Create Geofence</h1>
            <p className="text-gray-600 mt-1">
              Create a new geofence for the alert system
            </p>
          </div>
        </div>
...
export default GeofenceCreatePage;
