  ...
  const fetchInstitutes = useCallback(async () => {
    try {
      ...
      if (isAdmin) {
        ...
        response = await instituteService.getAllInstitutes();
      } else {
        // Regular user gets only their accessible institutes
      ...
      if (response.success && response.data) {
        ...
        if (!isAdmin) {
          const accessibleInstituteIds = accessibleInstitutes
            .filter(inst => inst.has_alert_system_access)
            .map(inst => inst.institute_id);
          institutes = institutes.filter(inst => accessibleInstituteIds.includes(inst.id));
        }
