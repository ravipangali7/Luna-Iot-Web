...
const GarbageIndexPage: React.FC = () => {
  const navigate = useNavigate();
  const { user } = useAuth();
  const { hasAccess, loading: accessLoading, isAdmin, accessibleInstitutes } = useGarbageAccess();
  ...
  const fetchInstitutes = useCallback(async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        const garbageInstituteIds = accessibleInstitutes
          .filter(inst => inst.has_garbage_access)
          .map(inst => inst.institute_id);
        
        // If no accessible institutes yet, wait for hook to load
        if (garbageInstituteIds.length === 0 && accessibleInstitutes.length === 0) {
          setInstitutes([]);
          setTotalCount(0);
          setTotalPages(0);
          setLoading(false);
        ...
        institutes = institutes.filter(inst => garbageInstituteIds.includes(inst.id));
  ...
  useEffect(() => {
    if (!accessLoading) {
      if (!hasAccess) {
        setError('You do not have access to the Garbage module. Please contact your administrator.');
        setLoading(false);
        return;
      }
      fetchInstitutes();
    ...
    return (
      <Container>
        <Alert variant="warning" className="mb-6">
          You do not have access to the Garbage module. Please contact your administrator.
        </Alert>
      </Container>
    );
  }
    ...
    <Container>
      <div className="mb-6">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Garbage Management</h1>
            <p className="text-gray-600 mt-1">
              Manage institutes with garbage vehicle services
            </p>
          </div>
        </div>
            ...
            {institutes.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  {searchTerm ? 'No institutes match your search criteria.' : 'No institutes with garbage module access found.'}
                </p>
              </div>
            ) : (
              <>
...
export default GarbageIndexPage;
