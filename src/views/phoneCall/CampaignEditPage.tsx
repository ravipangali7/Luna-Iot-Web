...
const CampaignEditPage: React.FC = () => {
  ...
  const loadCampaign = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        // Format schedule datetime (preserve time if present, otherwise add default time)
        let scheduleDateTime = '';
        if (campaignData.schedule) {
          // If schedule includes time, use it; otherwise add default time
          if (campaignData.schedule.includes('T')) {
            // Remove timezone and seconds if present, keep date and time
            scheduleDateTime = campaignData.schedule.substring(0, 16);
          } else {
            // Just date, add default time
            scheduleDateTime = `${campaignData.schedule}T00:00`;
          }
        }
        
        // Extract voice ID if it's an object
        let voiceId = null;
        if (campaignData.voice) {
          if (typeof campaignData.voice === 'object' && campaignData.voice.id) {
            voiceId = campaignData.voice.id;
          } else if (typeof campaignData.voice === 'number') {
            voiceId = campaignData.voice;
          }
        }
        
        setFormData({
          name: campaignData.name || '',
          services: campaignData.services || 'PHONE',
          user_phone: campaignData.user_phone || [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await phoneCallService.updateCampaign(parseInt(id), campaignData);
      
      if (result.success) {
        showSuccess('Campaign Updated', 'Campaign has been updated successfully.');
        ...
        showError('Campaign Update Failed', result.error || 'Failed to update campaign');
      }
    } catch (err) {
      setError('An unexpected error occurred');
    ...
    <Container>
      <div className="max-w-2xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-gray-600">Update campaign details</p>
        </div>
