...
const GpsDeviceIndexPage: React.FC = () => {
  ...
  const loadDevices = useCallback(async () => {
    try {
      ...
      // Load paginated GPS devices data
      const result = await deviceService.getGpsDevicesPaginated(currentPage);
      ...
      if (result.success && result.data) {
        ...
        setError(result.error || 'Failed to load GPS devices');
      }
    } catch (err) {
  ...
  const handleSearchFromUrl = useCallback(async (query: string, page: number = 1) => {
    try {
      ...
      setLoading(true);
      setError(null);
      const response = await deviceService.getGpsDevicesPaginated(page, query.trim());
  ...
  const handleSearch = async () => {
    ...
    try {
      setIsSearching(true);
      setLoading(true);
      const response = await deviceService.getGpsDevicesPaginated(1, searchInput.trim());
  ...
  const handleSearchPageChange = async (page: number) => {
    ...
    try {
      setIsSearching(true);
      setLoading(true);
      const response = await deviceService.getGpsDevicesPaginated(page, searchQuery);
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">Manage GPS tracking devices</p>
          </div>
        </div>
        ...
        <Card>
          {devices.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-500">No GPS devices found</p>
            </div>
          ) : (
            <>
...
export default GpsDeviceIndexPage;