...
import { communitySirenSwitchService, type CommunitySirenSwitch } from '../../../api/services/communitySirenService';
...
const SwitchIndexPage: React.FC = () => {
  ...
  const fetchSwitches = useCallback(async () => {
    try {
      ...
      const response = await communitySirenSwitchService.getByInstitute(Number(instituteId));
  ...
  const handleDelete = async (id: number, title: string) => {
    ...
    if (confirmed) {
      try {
        await communitySirenSwitchService.delete(id);
  ...
  const handleEdit = (id: number) => {
    navigate(`/community-siren/${instituteId}/switches/${id}/edit`);
  };

  // Handle create
  const handleCreate = () => {
    navigate(`/community-siren/${instituteId}/switches/create`);
  };
    ...
    <Container>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Community Siren Switches</h1>
            <p className="text-gray-600 mt-1">Manage community siren switches for this institute</p>
          </div>
          <Button
            variant="primary"
          ...
          <Card>
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">
                {searchTerm ? 'No switches match your search criteria.' : 'Get started by creating your first community siren switch.'}
              </p>
              {!searchTerm && (
          ...
          <Table striped hover>
            <TableHead>
              <TableRow>
                ...
                <TableHeader>Trigger (m)</TableHeader>
            ...
            <TableBody>
              {paginatedSwitches.map(switchItem => (
                <TableRow key={switchItem.id}>
                  ...
                  <TableCell className="text-gray-600">{switchItem.trigger}m</TableCell>
