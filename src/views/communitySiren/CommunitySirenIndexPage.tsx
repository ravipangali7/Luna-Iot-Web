...
const CommunitySirenIndexPage: React.FC = () => {
  ...
  const fetchInstitutes = useCallback(async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        const communitySirenInstituteIds = accessibleInstitutes
        ...
        if (communitySirenInstituteIds.length === 0 && accessibleInstitutes.length === 0) {
          setInstitutes([]);
          setTotalCount(0);
          setTotalPages(0);
          setLoading(false);
        ...
        institutes = institutes.filter(inst => communitySirenInstituteIds.includes(inst.id));
  ...
  useEffect(() => {
    if (!accessLoading) {
      if (!hasAccess) {
        setError('You do not have access to the Community Siren. Please contact your administrator.');
        setLoading(false);
        return;
      }
      fetchInstitutes();
    ...
    return (
      <Container>
        <Alert variant="warning" className="mb-6">
          You do not have access to the Community Siren. Please contact your administrator.
        </Alert>
      </Container>
    );
  }
    ...
    <Container>
      <div className="mb-6">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Community Siren</h1>
            <p className="text-gray-600 mt-1">
              Manage institutes with community siren access
            </p>
          </div>
        </div>
            ...
            {institutes.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  {searchTerm ? 'No institutes match your search criteria.' : 'No institutes with community siren access found.'}
                </p>
              </div>
            ) : (
              <>
...
export default CommunitySirenIndexPage;
