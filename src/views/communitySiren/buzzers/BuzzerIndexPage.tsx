...
import { communitySirenBuzzerService } from '../../../api/services/communitySirenService';
...
interface CommunitySirenBuzzer {
  id: number;
  title: string;
  device: number;
  device_imei: string;
...
const BuzzerIndexPage: React.FC = () => {
  ...
  const fetchBuzzers = useCallback(async () => {
    try {
      ...
      const response = await communitySirenBuzzerService.getByInstitute(Number(instituteId));
      setBuzzers(response || []);
    } catch (err: unknown) {
  ...
  const handleDelete = async (id: number, title: string) => {
    ...
    if (confirmed) {
      try {
        await communitySirenBuzzerService.delete(id);
  ...
  // Handle create
  const handleCreate = () => {
    navigate(`/community-siren/${instituteId}/buzzers/create`);
  };
    ...
    <Container>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Community Siren Buzzers</h1>
            <p className="text-gray-600 mt-1">Manage community siren buzzers for this institute</p>
          </div>
          <Button
            variant="primary"
          ...
          <Card>
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">
                {searchTerm ? 'No buzzers match your search criteria.' : 'Get started by creating your first community siren buzzer.'}
              </p>
              {!searchTerm && (
