...
import type { SubscriptionPlanFormData, Permission } from '../../types/subscriptionPlan';
...
const SubscriptionPlanCreatePage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await subscriptionPlanService.createSubscriptionPlan(formData);
      
      if (response.success) {
        showSuccess('Subscription plan created successfully');
        navigate('/subscription-plans');
      } else {
        setError(response.error || 'Failed to create subscription plan');
      }
    } catch (err) {
