...
const CommunitySirenHistoryViewPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { hasAccess, loading: accessLoading, isAdmin } = useCommunitySirenAccess();
  
  const [alertHistory, setAlertHistory] = useState<CommunitySirenHistory | null>(null);
  ...
  const fetchAlertHistory = useCallback(async () => {
    try {
      ...
      const data = await communitySirenHistoryService.getById(historyId);
  ...
  const handleStatusChange = async () => {
    ...
    if (newStatus) {
      try {
        setUpdating(true);
        const updatedHistory = await communitySirenHistoryService.updateStatus(alertHistory.id, { status: newStatus });
  ...
  const handleRemarksChange = async () => {
    ...
    if (newRemarks !== undefined) {
      try {
        setUpdating(true);
        const updatedHistory = await communitySirenHistoryService.update(alertHistory.id, { remarks: newRemarks });
        setAlertHistory(updatedHistory);
   ...
   const handleDelete = async () => {
     ...
     if (confirmed) {
       try {
         await communitySirenHistoryService.delete(alertHistory.id);
         showSuccess('Alert history deleted successfully');
         navigate('/community-siren-history');
       } catch (err) {
  ...
  const getSourceBadge = (source: string) => {
    switch (source.toLowerCase()) {
      case 'app':
        return <Badge variant="info" size="md">App</Badge>;
      case 'switch':
    ...
    <Container>
      <div className="mb-6">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Community Siren History Details</h1>
            <p className="text-gray-600 mt-1">
              View and manage community siren history record
            </p>
          </div>
          ...
          <div className="flex space-x-3">
            <Button
              variant="secondary"
              onClick={() => navigate('/community-siren-history')}
              icon={
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <p className="text-gray-900">{alertHistory.source_display || alertHistory.source}</p>
              </div>
              ...
              {alertHistory.secondary_phone && (
                <div>
                  <label className="text-sm font-medium text-gray-500">Secondary Phone</label>
                  <p className="text-gray-900">{alertHistory.secondary_phone}</p>
                </div>
              )}
              ...
              {alertHistory.member_name && (
                <div>
                  <label className="text-sm font-medium text-gray-500">Member (Triggered By)</label>
                  <div className="mt-1">
                    <p className="text-gray-900 font-medium">{alertHistory.member_name}</p>
                    {alertHistory.member_phone && (
                      <p className="text-gray-600 text-sm">{alertHistory.member_phone}</p>
                    )}
                  </div>
                </div>
              )}
            ...
            {/* Location Map */}
            {alertHistory.latitude && alertHistory.longitude && (
              <div className="mt-6 pt-6 border-t border-gray-200">
                <label className="text-sm font-medium text-gray-500 mb-2 block">Location</label>
                <div className="border border-gray-200 rounded-md overflow-hidden">
                  <AlertLocationMap 
                    latitude={Number(alertHistory.latitude)} 
                    longitude={Number(alertHistory.longitude)} 
                    height="300px" 
                  />
                </div>
                <p className="text-sm text-gray-600 mt-2">
                  Coordinates: {Number(alertHistory.latitude).toFixed(6)}, {Number(alertHistory.longitude).toFixed(6)}
                </p>
              </div>
            )}
            ...
            {alertHistory.images && (
              <div className="mt-6 pt-6 border-t border-gray-200">
                ...
                <div className="mt-2">
                  <img
                    src={`${API_CONFIG.BASE_URL}${alertHistory.images}`}
                    alt="Alert image"
...
export default CommunitySirenHistoryViewPage;
