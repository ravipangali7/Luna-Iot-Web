...
import type { InstituteModuleAccess } from '../types/publicVehicle';

interface UsePublicVehicleAccessReturn {
  hasAccess: boolean;
  loading: boolean;
  isAdmin: boolean;
...
export const usePublicVehicleAccess = (instituteId?: number): UsePublicVehicleAccessReturn => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const checkAccess = async () => {
      try {
        ...
        // Backend returns all institutes with public-vehicle module for admin, and only user's for others
        const response = await apiClient.get<{ success: boolean; data: InstituteModuleAccess[] }>('/api/core/institute/modules/public-vehicle-institutes/');
        ...
        console.error('Error checking public vehicle access:', error);
        setHasAccess(false);
        setAccessibleInstitutes([]);
      } finally {
        setLoading(false);
  ...
  const hasAccessToInstitute = useCallback((instituteId: number): boolean => {
    ...
    return accessibleInstitutes.some(inst => 
      inst.institute_id === instituteId && inst.has_public_vehicle_access
    );
